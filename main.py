...
class InputBox:
    ...
    def _update_from_mouse(self, mouse_x):
        ...
        if val > 0.99: self.text = f"{val:.8f}".rstrip('0')
        ...
        if self.is_speed_slider:
            try:
                ...
                if 0 <= val < 1.0:
                    ...
                    pygame.draw.rect(screen, COLOR_SLIDER, (self.rect.x, self.rect.y + self.rect.height - 4, fill_w, 4))
    ...
    def get_value(self):
        try:
            if not self.text or self.text == "-": return 0.0
            val = float(self.text)
            return max(0.0, val)
        except ValueError:
            return 0.0
...
class Simulation:
    ...
    def update(self, dt_real_seconds):
        ...
        # Convert real seconds to simulation years
        ...
        if self.direction == 1: # Outbound
            self.ship_x += move_px
            if self.ship_x >= self.end_x:
                self.ship_x = self.end_x
                self.direction = -1 # Turn back immediately
                
        elif self.direction == -1: # Return
            self.ship_x -= move_px
    ...
    def draw(self, screen):
        ...
        ship_w, ship_h = 25, 16
        pts = [(self.ship_x + ship_w/2 * self.direction, self.y_track), 
...
def create_inputs_for_sim(sim, base_y):
    # Line 1: Speed (slider), Conversions (auto), Distance
...
def init_app():
    ...
    # Simulation 1
    s1 = Simulation(1, 220, (255, 230, 0), 0.8, 4.0)
    sims.append(s1)
    sim_inputs.extend(create_inputs_for_sim(s1, s1.y_inputs_1))
    
    # Simulation 2
    s2 = Simulation(2, 570, (0, 240, 240), 0.999, 10.0)
    sims.append(s2)
    ...
    while running:
        ...
        for event in events:
            ...
            for box in sim_inputs:
                if box.handle_event(event): any_changed = True
            
            if global_input_dur.handle_event(event): any_changed = True
